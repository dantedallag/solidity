...
/// Accepts a reference to an evmc_message and a user-specified input
/// and zero initializes all fields except gas and input fields.
...
/// user input.
void initializeMessage(evmc_message& _msg, dev::bytes const& _input)
{
	...
	// Set flags to EVMC_STATIC to switch to static call mode
	_msg.flags = 0;
	// Gas available is the maximum value possible in EVM One
	_msg.gas = std::numeric_limits<int64_t>::max();
	_msg.input_data = _input.data();
	_msg.input_size = _input.size();
}
...
)
{
	...
	initializeMessage(message, _functionHash);
	message.destination = _deployedAddress;
	message.kind = EVMC_CALLCODE;
...
{
	evmc_message message;
	initializeMessage(message, _code);
...
DEFINE_PROTO_FUZZER(Contract const& _input)
{
	...
	std::cout << createResult.status_code << std::endl;
	solAssert(
